...
var ResponsibilityForm = t.struct({
...
class ResponsibilityView extends Component {
  ...
  constructor(props) {
    ...
    var docRef = db.collection('responsibilities').doc(this.props.navigation.state.params.id);
    docRef.get().then((doc) => {
      if (doc.exists) {
        ...
        this.toastify.show('Keine Befugnis gefunden', 1000);
  ...
  submitForm() {
    ...
    if (params.mode === 'edit') {
      ...
      if (formData) { // if validation fails, value will be null
        ...
        var docRef = db.collection("responsibilities").doc(`${params.id}`);
  ...
  removeResponsibility() {
    ...
    const params = navigation.state.params;
    const id = `${params.id}`;
    var deleteDoc = db.collection('responsibilities').doc(id).delete();
    navigation.goBack();
  }
  ...
  render() {
    if (this.state) {
      if (this.state.data) {
        ...
        if (params.mode === 'edit') {
          // View to Edit the Responsibility
          viewMode = <View style={Styles.ci_formContainer}>
            <Form
              ...
              value={data}
            />
            <TouchableHighlight style={styles.button} onPress={this.removeResponsibility} underlayColor='#99d9f4'>
