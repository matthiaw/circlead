...
class OrganisationView extends Component {
  ...
  constructor(props) {
    ...
    var docRef = db.collection('organisations').doc(this.props.navigation.state.params.id);
    docRef.get().then((doc) => {
      if (doc.exists) {
    ...
    //    this.toastify.show('Keine Funktion gefunden', 1000);
  ...
  submitForm() {
    ...
    if (params.mode === 'edit') {
      ...
      if (formData) { // if validation fails, value will be null
        ...
        var docRef = db.collection("organisations").doc(`${params.id}`);
  ...
  removeOrganisation() {
    ...
    const params = navigation.state.params;
    const id = `${params.id}`;
    var deleteDoc = db.collection('organisations').doc(id).delete();
    navigation.goBack();
  }
        ...
        if (params.mode === 'edit') {
          ...
          viewMode = <View style={Styles.ci_formContainer}>
            <Form
              ...
              value={data}
            />
            <TouchableHighlight style={styles.button} onPress={this.removeOrganisation} underlayColor='#99d9f4'>
