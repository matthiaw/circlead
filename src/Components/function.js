...
class FunctionView extends Component {
  ...
  constructor(props) {
    ...
    var docRef = db.collection('functions').doc(this.props.navigation.state.params.id);
    docRef.get().then((doc) => {
      if (doc.exists) {
  ...
  submitForm() {
    ...
    if (params.mode === 'edit') {
      ...
      if (formData) { // if validation fails, value will be null
        ...
        var docRef = db.collection("functions").doc(`${params.id}`);
  ...
  removeFunction() {
    ...
    const params = navigation.state.params;
    const id = `${params.id}`;
    var deleteDoc = db.collection('functions').doc(id).delete();
    navigation.goBack();
  }
        ...
        if (params.mode === 'edit') {
          ...
          viewMode = <View style={Styles.ci_formContainer}>
            <Form
              ...
              value={data}
            />
            <TouchableHighlight style={styles.button} onPress={this.removeFunction} underlayColor='#99d9f4'>
