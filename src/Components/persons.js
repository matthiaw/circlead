...
export default class Persons extends Component {
  ...
  setItemsFromFirestore() {
    var persons = db.collection("persons").get();
    persons.then((querySnapshot) => {
      // get children as an array
      var items = [];
      querySnapshot.forEach((child) => {
        items.push({
          title: `${child.data().firstname} ${child.data().familyname}`,
          description: ``,
          id: `${child.id}`
        });
      });
      ...
      headerRight: (
        <FontAwesome
          ...
          onPress={() => {
            ...
            var data = {
              id: `${id}`,
              familyname: `${id.substring(0,6)}`,
              firstname: 'Person',
              status: 'draft'
            };
            var setDoc = db.collection('persons').doc(id).set(data);
          }}
        />
      )
    };
