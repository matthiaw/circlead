...
class AuthorityView extends Component {
  ...
  constructor(props) {
    ...
    this.removeAuthority = this.removeAuthority.bind(this);

    var docRef = db.collection('authorities').doc(this.props.navigation.state.params.id);
    docRef.get().then((doc) => {
      if (doc.exists) {
  ...
  submitForm() {
    ...
    if (params.mode === 'edit') {
      ...
      if (formData) { // if validation fails, value will be null
        ...
        var docRef = db.collection("authorities").doc(`${params.id}`);
  ...
  removeAuthority() {
    ...
    const params = navigation.state.params;
    const id = `${params.id}`;
    var deleteDoc = db.collection('authorities').doc(id).delete();
    navigation.goBack();
  }
        ...
        if (params.mode === 'edit') {
          ...
          viewMode = <View style={Styles.ci_formContainer}>
            <Form
              ...
              value={data}
            />
            <TouchableHighlight style={styles.button} onPress={this.removeAuthority} underlayColor='#99d9f4'>
