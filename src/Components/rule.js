...
class RuleView extends Component {
  ...
  constructor(props) {
    ...
    var docRef = db.collection('rules').doc(this.props.navigation.state.params.id);
    docRef.get().then((doc) => {
      if (doc.exists) {
    ...
    //     this.toastify.show('Geladen', 1000);
      ...
      //  this.toastify.show('Kein Prinzip gefunden', 1000);
  ...
  submitForm() {
    ...
    if (params.mode === 'edit') {
      ...
      if (formData) { // if validation fails, value will be null
        ...
        var docRef = db.collection("rules").doc(`${params.id}`);
  ...
  removeRule() {
    ...
    const params = navigation.state.params;
    const id = `${params.id}`;
    var deleteDoc = db.collection('rules').doc(id).delete();
    navigation.goBack();
  }
        ...
        if (params.mode === 'edit') {
          ...
          viewMode = <View style={Styles.ci_formContainer}>
            <Form
              ...
              value={data}
            />
            <TouchableHighlight style={styles.button} onPress={this.removeRule} underlayColor='#99d9f4'>
