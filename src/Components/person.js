...
class PersonView extends Component {
  ...
  constructor(props) {
    ...
    var docRef = db.collection('persons').doc(this.props.navigation.state.params.id);
    docRef.get().then((doc) => {
      if (doc.exists) {
...
//      this.toastify.show('Laden fehlgeschlagen', 1000);
  ...
  submitForm() {
    ...
    if (params.mode === 'edit') {
      ...
      if (formData) { // if validation fails, value will be null
        ...
        var docRef = db.collection("persons").doc(`${params.id}`);
  ...
  removePerson() {
    ...
    const params = navigation.state.params;
    const id = `${params.id}`;
    var deleteDoc = db.collection('persons').doc(id).delete();
    navigation.goBack();
  }
        ...
        if (params.mode === 'edit') {
          ...
          viewMode = <View style={Styles.ci_formContainer}>
            <Form
              ...
              value={data}
            />
            <TouchableHighlight style={styles.button} onPress={this.removePerson} underlayColor='#99d9f4'>
          ...
          viewMode =
            <View style={Styles.ci_formContainer}>
              ...
              <Text style={Styles.ci_formLabel}>Vorname</Text>
              <Text style={Styles.ci_formText}>{data.firstname}</Text>
              <Text style={Styles.ci_formLabel}>Nachname</Text>
              <Text style={Styles.ci_formText}>{data.familyname}</Text>
